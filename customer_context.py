...
class CustomerStore:
    ...
    Tracks historical payments, calculates aggregates,
    and caches customer profiles.
    """
    ...
    def get_customer_context(self, customer_id: str) -> CustomerContext:
        ...
        # Use deterministic synthetic demo context so the UI
        # remains meaningful while the project uses synthetic data.
        if not row:
            return CustomerContext(
                ...
                average_transaction_value=3200.0,
                recent_failures=[],
                customer_tier="RELIABLE_REPEAT",
            )
...
def build_customer_context(
    ...
    """
    Deterministic service that derives or looks up
    a customer profile for a given PaymentEvent.
    """
    ...
    if not customer_id:
        ...
        customer_hash = hashlib.md5(group.encode()).hexdigest()[:8]
        customer_id = f"cust_{customer_hash}"
